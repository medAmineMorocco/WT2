import { Button, ConfigProvider, Layout, Typography, theme, Row, Col, Empty, Tooltip } from 'antd';
      ...
      <Layout>
        <Layout style={{ height: '100vh' }}>
          <Layout style={{ padding: '24px' }}>
            ...
            >
              <div style={{ display: 'flex', width: '100%', backgroundColor: isDarkMode ? 'rgb(20, 20, 20)' : colorBgContainer, zIndex: 1, position: 'sticky', top: 0 }}>
                <Button type="text" icon={<CloseOutlined />} onClick={onClose}></Button>
                <div style={{ display: 'flex', alignItems: 'center'}}><strong>Logs</strong></div>
                <Tooltip
                  title="Clear Logs"
                  placement="left"
                  mouseEnterDelay={0}
                  mouseLeaveDelay={0}
                >
                <Button type="text" icon={<ClearOutlined />} style={{ marginLeft: 'auto'}} onClick={clearLogs}></Button>
                </Tooltip>
              </div>
                    ...
                    <React.Fragment key={index}>
                      ...
                      <Col span={17}>
                        {log.level === 'ERROR' || log.level === 'WARNING' ? (
                          <Typography.Text type="danger" copyable>{log.message}</Typography.Text>
                        ) : (
                          <Typography.Text copyable>{log.message}</Typography.Text>
                        )}
                      </Col>
                    </React.Fragment>
                  ))}
